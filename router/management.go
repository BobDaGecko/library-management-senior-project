...
// ... (other functions unchanged: HandleManagementUsers, HandleManagementUserDetail, HandleManagementOverdue, HandleManagementTransactions, parsePageParam, HandleManagementBooksAdd)
...
// Supports string, int, bool, time.Time. Extend as needed for slices.
	...
	for key, vals := range form {
		...
		for i := 0; i < t.NumField(); i++ {
			...
			if strings.EqualFold(sf.Name, key) {
				...
				// Note: For SqlStringList ([]string), form can send comma-separated or handle separately if needed.
				}
				break
			}
		}
